    ...
    for ang_file in rawtlt_files:
        with open(ang_file) as f:
            angles[Path(ang_file).stem] = [
                float(line) for line in f.readlines() if line
            ]

    datadir = Path(ang_file).parent

    for basename, angles_ in track(angles.items(), description="Spoofing..."):
        mdoc = Mdoc(
            titles=["[T = Mdoc spoofed by stemia]"],
            global_data=MdocGlobalData(ImageFile=basename, PixelSpacing=pixel_size),
            section_data=[],
        )
        for img, angle in zip(
            sorted(datadir.glob(f"{basename}*.{extension}")), angles_
        ):
