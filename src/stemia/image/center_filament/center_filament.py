...
@click.argument(
    "output", type=click.Path(dir_okay=False, resolve_path=True), required=False
)
@click.option(
    "-s",
    "--update-star",
...
@click.option(
    ...
    help="where to put the updated version of the star file. "
    "Only used if -s is passed [default: STARFILE_centered.star]",
)
@click.option(
    "--update-by",
    type=click.Choice(["class", "particle"]),
    default="class",
    show_default=True,
...
@click.option(
    ...
    "--n-filaments",
    default=2,
    help="number of filaments on the image",
    show_default=True,
)
@click.option(
    "-p",
    "--percentile",
    default=85,
    help="percentile for binarisation",
    show_default=True,
)
def cli(
    input, output, starfile, star_output, update_by, n_filaments, percentile, overwrite
):
    """
    ...
    from ..utils.io_ import read_mrc, read_particle_star, write_mrc, write_particle_star
    ...
    out_imgs, shifts, angles = center_filaments(
        imgs, n_filaments=n_filaments, percentile=percentile
    )
